"""
Pandas Skill —— execute任意 pandas 代码
"""
import sys
import io
import ast
    ...
    try:
        ...
        if last_result is not None:
            if isinstance(last_result, pd.DataFrame):
                result["dataframe_preview"] = {
                    "shape": list(last_result.shape),
                    "columns": list(last_result.columns),
                    "head": last_result.head(10).to_string(),
                }
            elif isinstance(last_result, (pd.Series,)):
                result["result"] = repr(last_result)
            elif isinstance(last_result, (int, float, str, bool, list, dict, tuple)):
                result["result"] = repr(last_result)
            else:
                result["result"] = str(last_result)[:2000]

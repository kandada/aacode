...
def thinking_kwargs_for_mode(mode: str) -> Optional[Dict[str, Any]]:
...
def next_thinking_mode(current_mode: str) -> Optional[str]:
    ...
    Returns:
        下个模式；None 表示已是终态（"none"），调用方应停止并 fallback 到非流式。
    """
    try:
...
def is_thinking_mode_rejected(error: Exception, mode: str) -> bool:
    ...
    关键边界：错误信息若包含其他具体 mode 名（如 ``"thinking.type.adaptive"``
    被拒而我们正在尝试 ``enabled``），不应判为"当前 mode 被拒"——那是别的 mode 的事。
    """
    msg = str(error).lower()
    ...
    # 2. 检查是否提到了其他具体 mode 名（说明错误指向的是其他 mode）
    #    如果提到了其他 mode 的精确错误，我们当前 mode 没有被拒

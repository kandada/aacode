...
class ToolParameter:
    ...
    def validate(self, value: Any) -> Tuple[bool, Optional[str]]:
        ...
        if value is not None and not isinstance(value, self.type):
            return False, f"参数 '{self.name}' 期望类型为 {self.type.__name__}，实际类型为 {type(value).__name__}"
        return True, None
    ...
    def validate(self, input_params: Dict) -> Tuple[bool, Optional[str]]:
        ...
        if missing_params:
            ...
            for param_name in missing_params:
                ...
                aliases_str = f" (别名: {', '.join(param.aliases)})" if param.aliases else ""
...
class ToolRegistry:
    ...
    def suggest_similar_tools(self, tool_name: str, max_suggestions: int = 3) -> List[str]:
        """建议相似的工具名称"""

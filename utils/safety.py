...
class SafetyGuard:
    ...
    def __init__(self, project_path: Path, interactive: bool = True):
        ...
        self.allowed_commands = {
            # 基础命令
            'ls', 'cd', 'pwd', 'cat', 'echo', 'grep', 'glob', 'find', 'sed', 'awk',
            'mkdir', 'rmdir', 'touch', 'cp', 'mv', 'rm',  # rm需特别检查
            'head', 'tail', 'less', 'more', 'wc', 'sort', 'uniq',
            'which', 'whereis', 'file', 'stat', 'basename', 'dirname',
            
            # Python生态
            'python', 'python3', 'pip', 'pip3', 'virtualenv', 'venv', 
            'poetry', 'pipenv', 'conda', 'pytest', 'unittest', 'nose', 'tox',
            'django-admin', 'django', 'flask', 'fastapi', 'uvicorn',
            'gunicorn', 'celery', 'celery-worker',
            
            # Node.js生态
            'node', 'npm', 'npx', 'yarn', 'pnpm', 'express-generator', 'nest', 'strapi',

            # 版本控制
            'git', 'svn', 'hg', 'pre-commit', 'git-lfs', 'git-flow', 'husky',
            
            # 网络工具
            'curl', 'wget', 'http', 'httpie', 'scp', 'rsync', 'ssh', 'ping', 'traceroute', 'netstat', 'sftp', 'sshfs', 'rclone', 'nc',
            ...
            # 包管理
            'apt', 'apt-get', 'dpkg', 'yum', 'dnf', 'brew', 'pacman',
            'systemctl', 'service',
            'sudo',  # 权限提升（需特别检查）
            ...
            # 文本处理
            'tr', 'cut', 'paste', 'join', 'diff', 'patch', 'jq', 'yq', 'xmlstarlet', 'csvkit', 'pandoc',

            # 文件操作扩展
            'tree', 'exa', 'bat', 'fzf', 'ripgrep', 'silversearcher-ag',
            'entr', 'inotifywait', 'watchexec',

            # 虚拟化和容器
            'vagrant', 'virtualbox', 'qemu', 'kvm', 'lxc', 'lxd', 'buildah', 'skopeo', 'nerdctl',

            # Docker生态
            'docker', 'docker-compose', 'docker-build', 'docker-run',
            'docker-ps', 'docker-images', 'docker-logs', 'docker-exec',
            'docker-stop', 'docker-rm', 'docker-rmi',
            'podman', 'podman-compose',

            # 数据库工具
            'psql', 'mysql', 'sqlite3', 'mongosh', 'redis-cli', 'pg_dump', 'mysqldump', 'mongoexport',  'redis-server',

            # 系统监控
            'htop', 'iotop', 'nethogs', 'glances', 'ncdu',

            # 性能分析
            'perf', 'strace', 'ltrace', 'valgrind',
            ...
            # 图像工具
            'ffmpeg', 'convert', 'identify', 'ffprobe',

            # 其他常用工具
            'date', 'cal', 'bc', 'expr', 'seq', 'yes', 'true', 'false',
            'sleep', 'timeout', 'watch', 'time',
            'env', 'printenv', 'set', 'unset',
            'clear', 'reset', 'tput',

            # 其他实用工具
            'tmux', 'screen', 'byobu', 'zellij', 'parallel', 'xargs', 'tee', 'sponge',
            'units', 'bc', 'dc', 'calc', 'neofetch', 'screenfetch', 'asciin', 'asciinema',
            'nvidia',  'byobu', 'zellij', 'fzf', 'exa', 'lsd', 'tldr', 'cheat', 'navi',
            'starship', 'oh-my-zsh', 'powerline',

            # 日志查看
            'journalctl', 'dmesg', 'tail', 'less +F',
            'multitail', 'lnav',
    ...
    def is_safe_path(self, path: Path) -> bool:
        ...
        try:
            ...
            allowed_system_paths = [
                '/tmp', '/var/tmp', '/private/tmp',  # 临时目录（包括macOS的/private/tmp）
                '/usr/share', '/usr/local/share',  # 共享数据
                '/etc/passwd', '/etc/group',  # 只读系统文件
                '/proc/self', '/proc/cpuinfo', '/proc/meminfo',  # 只读系统信息
            ]
            
            # 检查原始路径和解析后的路径
            for allowed_path in allowed_system_paths:
                if str(resolved).startswith(allowed_path) or path_str.startswith(allowed_path):
                    return True
    ...
    def _build_result(self, allowed: bool, reason: str, risk_level: str = "", 
                     needs_confirmation: bool = False, **kwargs) -> Dict[str, Any]:
        """构建检查结果字典
        ...
        result = {
            "allowed": allowed,
            "reason": reason,
            "risk_level": risk_level or (self.RISK_SAFE if allowed else self.RISK_DANGEROUS),
            "needs_confirmation": needs_confirmation if needs_confirmation is not None else False
        }
        result.update(kwargs)
        return result
    
    def _assess_risk_level(self, cmd_name: str, command: str, parts: List[str]) -> Tuple[str, str]:
        """评估命令风险等级
    ...
    def check_command(self, command: str, ask_confirmation: bool = True) -> Dict[str, Any]:
        """检查命令安全性
        ...
        try:
                ...
                if has_dangerous_option:
                    ...
                    if ask_confirmation and self.interactive:
                        ...
                        if not self._ask_user_confirmation(
                            command, 
                            f"rm -rf 递归删除操作，将删除: {target_desc}"
                        ):
            ...
            if cmd_name == 'sudo':
                # 只允许特定的sudo命令
                allowed_sudo_commands = {'apt', 'apt-get', 'dpkg', 'systemctl', 'service', 'pip', 'pip3', 'npm', 'yarn'}
                if len(parts) > 1:
                    ...
                    if any(install_keyword in ' '.join(parts).lower() for install_keyword in ['install', 'add', 'update', 'upgrade']):
                        if ask_confirmation and self.interactive:
                            if not self._ask_user_confirmation(command, "sudo权限操作，可能修改系统配置"):
                                return {"allowed": False, "reason": "用户取消操作"}
                            print(f"✅ 用户确认sudo操作")
                            return {"allowed": True, "reason": "软件包安装操作（用户已确认）"}
                        else:
                            # 非交互模式，直接允许
            ...
            if cmd_name in ['chmod', 'chown']:
                ...
                if len(parts) > 1:
                    for part in parts[1:]:
                        if part.startswith('/') and not part.startswith(self.project_root):
                            return {
                                "allowed": False,
                        ...
                        if cmd_name == 'chmod' and '777' in part:
                            if ask_confirmation and self.interactive:
                                if not self._ask_user_confirmation(command, "chmod 777权限过于宽松，存在安全风险"):
                                    return {"allowed": False, "reason": "用户取消操作"}
                                return {"allowed": True, "reason": "chmod 777操作（用户已确认）"}
                            else:
                                # 非交互模式，允许但警告
                                return {"allowed": True, "reason": "chmod 777操作（项目内）"}
                
                # 项目内的权限修改，在交互模式下询问确认
                if ask_confirmation and self.interactive:
                    if not self._ask_user_confirmation(command, f"{cmd_name}权限修改操作"):
                        return {"allowed": False, "reason": "用户取消操作"}
            ...
            if cmd_name in ['pip', 'pip3']:
                ...
                if len(parts) > 1:
                    ...
                    # 只禁止明确危险的操作
                    forbidden_pip_actions = {'download', 'wheel'}  # 这些可能下载大量文件
                ...
                if '..' in part or part.startswith('/'):
                    try:
                        ...
                        if not self.is_safe_path(test_path):
                            # 对于某些只读命令，允许访问系统文件
                            readonly_commands = {
                                'ls', 'cat', 'file', 'stat', 'head', 'tail', 'less', 'more',
                            ...
                            if cmd_name in readonly_commands:
                                print(f"⚠️  警告: {cmd_name}命令访问项目目录外路径: {part}")
                                continue
                            
                            # 对于临时目录操作，允许
                            tmp_prefixes = ['/tmp', '/var/tmp', '/private/tmp']
                            if any(str(test_path).startswith(prefix) for prefix in tmp_prefixes):
                                if cmd_name in ['mkdir', 'touch', 'rm', 'cp', 'mv']:
                                    print(f"⚠️  警告: {cmd_name}命令在临时目录操作: {part}")
                                    continue
                            
                            # 对于包管理器，允许系统路径
                            package_managers = {'apt', 'apt-get', 'dpkg', 'yum', 'dnf', 'brew', 'pip', 'pip3', 'npm', 'yarn'}

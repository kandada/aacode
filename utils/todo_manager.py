...
class TodoManager:
    ...
    async def create_todo_list(self, task_description: str, project_name: Optional[str] = None, context_manager: Any = None) -> str:
        """
        创建待办清单
...
"""
    ...
    async def add_todo_item(self, item: str, priority: str = "medium", category: str = "任务") -> bool:
        """
        添加待办事项 - 优化1：增量追加，更高效
    ...
    async def mark_todo_completed(self, item_pattern: str) -> bool:
        ...
        for i, line in enumerate(lines):
            if line.strip().startswith("- [ ]") and item_pattern.lower() in line.lower():
                # 标记为完成
                ...
                try:
                    ...
                    async with aiofiles.open(file_path, 'r', encoding='utf-8') as f:
                        ...
                        if second_line.startswith("**创建时间**: "):
                            created_time = second_line.replace("**创建时间**: ", "").strip()

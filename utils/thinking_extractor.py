...
- ``extract_thinking_from_text``: 非流式文本的一次性提取（用于 fallback / 回放）
...
def extract_thinking_from_text(text: str) -> Tuple[str, str]:
    """从完整（非流式）文本中一次性提取 thinking 与正文。

    仅走标签策略，因为非流式响应中如有 ``reasoning_content`` 字段，通常
    已在调用方独立处理。这里用于补齐 ``<think>...</think>`` 标签解析的 fallback。

    Returns:
        (thinking, content) —— 已剥离标签的干净正文与提取出的思考内容。
    """
    parser = _TagParser()
    think, content = parser.feed(text or "")
    f_think, f_content = parser.finalize()
    return (think + f_think, content + f_content)
...
class ContextManager:
            ...
            if init_file.exists():
                try:
                    ...
                    if init_content.strip():
                        context_parts.append(f"## 📋 项目初始化指令 (init.md)\n{init_content[:1500]}")
                    else:
            ...
            if errors_file.exists():
                try:
                    ...
                    if errors and errors.strip():
                        context_parts.append(f"## ⚠️ 重要错误历史（避免重复）\n{errors[-800:]}")  # 最近800字符
                except Exception:
                    # 静默失败
                    pass
        except Exception:
    ...
    async def update(self, observation: str):
        ...
        if observation and len(observation) > 0:
            ...
            if any(keyword in observation.lower() for keyword in ['错误', 'error', '失败', 'failed', '警告', 'warning']):
                errors_file = self.context_dir / "important_errors.txt"
                try:
                    # 追加模式，保留历史
                    existing_errors = ""
            ...
            if step.actions:
                for j, action_item in enumerate(step.actions, 1):
                    ...
                    if action_item.observation:
                        history_content += f"**观察 {j}**: {action_item.observation[:100]}...\n"
            history_content += "\n"

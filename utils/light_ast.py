...
class LightAST:
    ...
    def _convert_node(self, node: ast.AST, parent: Optional[ASTNode] = None) -> ASTNode:
        ...
        if isinstance(node, ast.Module):
            ...
            ast_node.attributes.update(
                {
                    ...
                    "returns": (
                        self._expr_to_str(node.returns) if node.returns else None
                    ),
                }
            )
            for child in node.body:
            ...
            ast_node.attributes["value"] = (
                self._expr_to_str(node.value) if node.value else None
            )
    ...
    def _expr_to_str(self, expr: ast.AST) -> str:
        ...
        try:
            ...
            if len(lines) == 1:
                start_col = getattr(expr, "col_offset", 0)
                end_col = getattr(expr, "end_col_offset", len(lines[0]))
                ...
                for i, line in enumerate(lines):
                    if i == 0:
                        ...
                        end_col = getattr(expr, "end_col_offset", len(line))
    ...
    def find_functions(self) -> List[ASTNode]:
        ...
        if not self.root:
            return []
        return self.root.find_nodes("FunctionDef") + self.root.find_nodes(
            "AsyncFunctionDef"
        )
...
class IncrementalASTHelper:
        ...
        if update_type == "line_update" and line_number:
            ...
            if nodes_in_range:
                ...
                result["suggestions"].append(
                    f"替换范围内包含{len(nodes_in_range)}个AST节点"
                )
    ...
    @staticmethod
    def validate_update(
        code: str, new_code: str, update_type: str, **kwargs
    ) -> Dict[str, Any]:
        """
        验证更新
    ...
    def generate_update_plan(code: str, update_type: str, **kwargs) -> Dict[str, Any]:
        ...
        validation = IncrementalASTHelper.validate_update(
            code, kwargs.get("new_content", ""), update_type, **kwargs
        )
        ...
        if update_type == "line_update":
            ...
            plan["steps"].append(
                {"action": "append_to_file", "description": "追加到文件末尾"}
            )

        elif update_type == "prepend":
            plan["steps"].append(
                {"action": "prepend_to_file", "description": "插入到文件开头"}
            )
...
def analyze_code_structure(code: str) -> Dict[str, Any]:
    ...
    for imp in ast_tool.find_imports():
        if imp.node_type == "Import":
            imports.append({"type": "import", "names": imp.attributes.get("names", [])})
        elif imp.node_type == "ImportFrom":
            imports.append(
                {
                    "type": "import_from",

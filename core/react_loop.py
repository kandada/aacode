...
📚 多读多思考原则（重要！）：
1. **先读后写**: 在编写代码前，务必先阅读项目中的相关文档和现有代码
   - 使用 read_file 工具读取 init.md、README.md 等文档
   - 使用 search_files 工具搜索相关代码和配置
   - 使用 list_files 工具了解项目结构
   - 善用run_shell使用计算机能力，善用glob、grep、curl等，支持多数常见命令
2. **充分理解**: 理解项目的架构、约定、依赖关系后再动手
3. **搜索资料**: 遇到不熟悉的技术，使用 search_web 工具搜索最佳实践
4. **参考现有**: 优先参考项目中已有的代码风格和模式
5. **避免重复错误**: 查看上下文中的"重要错误历史"，避免重复之前的错误

示例思考流程：
"我需要完成这个Web应用开发任务。首先，让我读取init.md了解项目规范，然后查看现有的代码结构，搜索Flask最佳实践。理解清楚后，我将分三步实现：1. 创建用户模型，2. 实现认证API，3. 添加数据处理接口。"
...
多语言注意事项：
- 项目可能包含多种编程语言，请根据文件扩展名使用正确的语法
- 对于非Python代码，遵循相应语言的最佳实践和约定
- 跨语言交互时注意接口兼容性和数据格式
"""
        ...
        for iteration in range(self.max_iterations):
            ...
            print(f"💭 思考: {thought[:500]}..." if len(thought) > 100 else f"💭 思考: {thought}")

            # 检查是否完成（没有action表示任务完成）
            if not actions or await self._is_task_completed(thought, actions[0].action if actions else None, task_description):
                print("✅ 任务完成")
            ...
            for i, action_item in enumerate(actions):
                ...
                while retry_count < max_retries:
                    try:
                        # 获取完整的工具执行结果
                        full_result = await self._execute_action_internal(action_item.action, action_item.action_input)
                        ...
                        if observation and isinstance(observation, str):
                            if "错误" in observation or "error" in observation.lower():
                                # 检查是否是可重试的错误
                                retryable_errors = ["timeout", "connection", "temporary", "暂时"]
                                if any(err in observation.lower() for err in retryable_errors):
                                    retry_count += 1
                                    if retry_count < max_retries:
                                        print(f"⚠️  动作失败，{retry_count}/{max_retries} 次重试...")
                        ...
                        retry_count += 1
                        if retry_count < max_retries:
                            print(f"⚠️  动作异常，{retry_count}/{max_retries} 次重试: {str(e)}")
                ...
                all_observations_for_display.append(observation_for_display)  # 用户看到简化版本
                ...
                if self.logger:
                    await self.logger.log_tool_call(
                        ...
                        success=not (observation.startswith("错误") or "error" in observation.lower()),
                        metadata={"retry_count": retry_count} if retry_count > 0 else None
                    )
            
            # 合并所有观察结果（Agent获取完整内容）
            observation = "\n".join([f"动作 {i+1} 结果: {obs}" for i, obs in enumerate(all_observations)])
            
            # 合并显示版本（用户看到简化版本）
            observation_for_display = "\n".join([f"动作 {i+1} 结果: {obs}" for i, obs in enumerate(all_observations_for_display)])
            
            # 上下文一致性检查
            await self._validate_context_consistency(all_observations, all_observations_for_display, messages)
            ...
            trigger_tokens = self.context_config.compact_trigger_tokens if self.context_config else 8000
        ...
        if self.logger:
            await self.logger.finish_task(
                ...
                total_time=total_time,
                summary={"last_thought": self.steps[-1].thought if self.steps else None}
            )
    ...
    async def _is_task_completed(self, thought: str, first_action: Optional[str], task_description: str) -> bool:
        """
        使用模型判断任务是否已完成
...
回答："""
    ...
    async def _check_recent_errors(self) -> bool:
        ...
        error_keywords = [
            ...
            '错误', '异常', '失败', 'importerror', 'syntaxerror',
            ...
            if step.actions:
                for action in step.actions:
                    ...
                    if action.observation:
                        ...
                        if any(kw in action.observation.lower() for kw in ['error', '错误', 'failed', '失败']):
    ...
    def _parse_response(self, response: str) -> tuple:
        ...
        for pattern in json_patterns:
            ...
            if json_match:
                ...
                try:
                    ...
                    thought = data.get('thought') or data.get('thinking') or data.get('reasoning') or ''
                    ...
                    for action_data in actions_data:
                        if isinstance(action_data, dict) and 'action' in action_data:
                            ...
                            action_input = action_data.get('action_input') or action_data.get('input') or {}
        ...
        while i < len(lines):
            ...
            action_match = re.match(r'Action\s*(\d+)?[:\s]+(?!Input)(.+)', line, re.IGNORECASE)
            ...
            if action_match:
                ...
                # 跳过空的action名称或只有冒号
                if not action_name or action_name == ':' or action_name.lower() == 'input':
                    i += 1
                    continue
                ...
                for j in range(i + 1, min(i + 11, len(lines))):
                    ...
                    # 如果遇到下一个Action，停止查找
                    if re.match(r'Action\s*(\d+)?[:\s]+(?!Input)', input_line, re.IGNORECASE):
                        break
                    
                    # 匹配Action Input行
                    input_match = re.match(r'Action\s+Input\s*(\d+)?[:\s]+(.+)', input_line, re.IGNORECASE)
                    ...
                    if input_match:
                        ...
                        if input_text.startswith('{'):
                            try:
                                # 清理可能的markdown残留
                                clean_input = input_text.replace('```json', '').replace('```', '').strip()
                                # 尝试修复JSON格式
    ...
    def _format_observation_for_display(self, action: str, result: Any) -> str:
        ...
        # 判断内容类型
        is_code_content = any(indicator in result_str[:200] for indicator in [
        ...
        is_test_output = any(indicator in result_str[:500] for indicator in [
            'test session starts', 'PASSED', 'FAILED', 'pytest',
            'unittest', 'Test', 'Traceback', 'AssertionError',
        ...
        if len(result_str) > medium_threshold:
            return result_str[:medium_threshold] + f"...\n\n（已截断，共{len(result_str)}字符）"
    ...
    async def _execute_action_internal(self, action: str, action_input: Dict) -> str:
        ...
        try:
            ...
            if len(result_str) > truncate_threshold:
                try:
                    output_file = await self.context_manager.save_large_output(result_str, f"{action}_output.txt")
                    # 返回预览内容
                    preview = result_str[:preview_length]
                    ...
                    if is_test_output and settings.output.test_summary_enabled:
                        ...
                        for line in result_str.split('\n'):
                            if any(keyword in line for keyword in ['passed', 'failed', 'error', 'PASSED', 'FAILED', 'ERROR', '===', 'collected']):
            ...
            elif "unexpected" in error_str.lower() or "got an unexpected" in error_str.lower():
    ...
    async def _compact_context(self, messages: List[Dict]):
        ...
        # 从配置获取参数
        keep_messages = self.context_config.compact_keep_messages if self.context_config else 20
        keep_rounds = self.context_config.compact_keep_rounds if self.context_config else 8
        summary_steps = self.context_config.compact_summary_steps if self.context_config else 10
        protect_first_rounds = self.context_config.compact_protect_first_rounds if self.context_config else 3
        ...
        first_rounds_messages = messages[2:2 + protect_first_rounds * 2]  # 前N轮对话（任务规划）
        recent_messages = messages[-keep_rounds * 2:]  # 最近N轮对话
        
        # 中间消息：跳过系统、前N轮、最近N轮
        middle_start = 2 + protect_first_rounds * 2
        middle_end = -keep_rounds * 2
        middle_messages = messages[middle_start:middle_end] if len(messages) > middle_start + keep_rounds * 2 else []
        ...
        # 使用模型生成三块智能摘要
        try:
            summaries = await self._generate_three_part_summary(middle_messages, self.steps[-summary_steps:])
        except Exception as e:
...
- 避免重复已完成的工作"""
        ...
        print(f"   Token数: {old_tokens} → {new_tokens} (减少 {old_tokens - new_tokens}, {(old_tokens - new_tokens) / old_tokens * 100:.1f}%)")
    ...
    async def _compact_file_contents(self, messages: List[Dict]) -> List[Dict]:
        ...
        for msg in messages:
            ...
            if len(content) > 500:
                ...
                # 策略1：匹配代码块（```...```）超过500字符的
                code_blocks = list(re.finditer(r'```[\s\S]{500,}?```', new_content, re.DOTALL))
                ...
                for match in code_blocks:
                    ...
                    try:
                        ...
                        # 生成智能摘要
                        summary = self._generate_content_summary(content_block, content_type)
                ...
                if not content_modified and len(content) > 1500:
                    ...
                    if '匹配' in content or 'search' in content.lower():
                        ...
                        try:
                            ...
                            summary = self._generate_content_summary(content, content_type)
...
💡 如需查看完整内容，使用 read_file 工具读取归档文件，或自己自主搜索需要的文件"""
    ...
    def _detect_content_type(self, full_content: str, content_block: str) -> str:
        ...
        # 检查上下文关键词
        if any(kw in full_lower for kw in ['文件内容', 'file content', '读取文件', 'read_file']):
            return 'file_content'
        elif any(kw in full_lower for kw in ['执行命令', 'run_shell', '命令输出', 'command output', 'stdout']):
            return 'shell_output'
        elif any(kw in full_lower for kw in ['搜索结果', 'search_files', 'search results']):
            return 'search_results'
        elif any(kw in full_lower for kw in ['文件列表', 'list_files', 'file list']):
            return 'file_list'
        elif any(kw in full_lower for kw in ['执行结果', 'execute_python', '代码输出', 'code output']):
            return 'code_output'
        else:
            # 默认类型
            return 'content'
    ...
    def _extract_identifier(self, full_content: str, content_type: str) -> str:
        ...
        if content_type == 'file_content':
            ...
            # 策略2: 查找明确的文件路径标记
            match = re.search(r'(?:文件路径|file path|读取文件|read_file)[:\s]+([^\n\s]+\.[\w]+)', full_content, re.IGNORECASE)
            if match:
                return match.group(1).strip()
            
            # 策略3: 查找任何看起来像文件路径的内容（包含扩展名）
            match = re.search(r'([a-zA-Z0-9_/\-\.]+\.(?:py|js|ts|md|txt|json|yaml|yml|csv|html|css))', full_content, re.IGNORECASE)
            if match:
                return match.group(1).strip()
            ...
            # 提取命令
            match = re.search(r'(?:命令|command)[:\s]+([^\n]+)', full_content, re.IGNORECASE)
            if match:
                cmd = match.group(1).strip()
                # 只保留命令的前30个字符，清理特殊字符
                cmd = cmd[:30].replace(' ', '_').replace('/', '_').replace('|', '_').replace('>', '_')
                return cmd
            return "unknown_cmd"
        
        elif content_type == 'search_results':
            # 提取搜索查询
            match = re.search(r'(?:搜索|search|query)[:\s]+([^\n]+)', full_content, re.IGNORECASE)
            if match:
                query = match.group(1).strip()[:30].replace(' ', '_').replace('"', '').replace("'", '')
                return query
    ...
    def _generate_content_summary(self, content_block: str, content_type: str) -> str:
        ...
        if content_type == 'shell_output':
            ...
            elif 'success' in content_block.lower() or 'passed' in content_block.lower():
    ...
    async def _generate_intelligent_summary(self, middle_messages: List[Dict], recent_steps: List[ReActStep]) -> str:
        """使用模型生成智能摘要"""
        # 构建摘要请求
...
"""
    ...
    async def _generate_three_part_summary(self, middle_messages: List[Dict], recent_steps: List[ReActStep]) -> Dict[str, str]:
        """使用模型生成三块分类摘要"""
...
请返回JSON格式的三块摘要："""
        ...
        try:
            ...
            if summaries and isinstance(summaries, dict):
                return {
                    ...
                    "tool_execution_summary": summaries.get("tool_execution_summary", ""),
        ...
        try:
            ...
            if isinstance(observation, dict):
                ...
                if 'returncode' in observation:
                    ...
                    if stderr and any(err in stderr.lower() for err in ['error', 'exception', 'traceback', 'failed']):
    ...
    def _extract_error_info(self, observation: str) -> tuple:
        ...
        if '命令执行失败' in observation or 'command failed' in observation.lower():
            ...
            if exitcode_match:
                ...
                # 提取 stderr 或错误消息
                stderr_match = re.search(r'错误输出[:\s]+(.+)', observation, re.IGNORECASE | re.DOTALL)
                if stderr_match:
        ...
        if 'returncode' in observation.lower() or 'exit code' in observation.lower():
            stderr_match = re.search(r'stderr[:\s]+(.+)', observation, re.IGNORECASE | re.DOTALL)
            if stderr_match:
                return "命令执行失败", stderr_match.group(1).strip()[:150]
            return "命令执行失败", "命令返回非零退出码"
        
        # 4. 尝试匹配参数错误
        if '参数验证失败' in observation or 'parameter validation failed' in observation.lower():
            # 提取参数名
    ...
    async def _generate_summary(self, recent_steps: List[ReActStep]) -> str:
        ...
        for i, step in enumerate(recent_steps):
            ...
            thought_preview = step.thought[:200] + ("..." if len(step.thought) > 200 else "")
            summary_parts.append(f"\n### 步骤 {i + 1}")
            summary_parts.append(f"**思考**: {thought_preview}")
            
            if step.actions:
                summary_parts.append(f"**动作**: {', '.join([a.action for a in step.actions])}")
                
                # 添加关键观察结果
                for j, action in enumerate(step.actions):
                    if action.observation:
                        # 保留观察结果的前150字符
                        obs_preview = action.observation[:150] + ("..." if len(action.observation) > 150 else "")
    ...
    async def _validate_context_consistency(self, 
        ...
        if len(all_observations) != len(all_observations_for_display):
            ...
            print(f"   Agent观察数: {len(all_observations)}, 用户观察数: {len(all_observations_for_display)}")
        ...
        if current_tokens > 5000:  # 警告阈值
            ...
            # 显示消息分布
            system_tokens = self._estimate_tokens([msg for msg in messages if msg.get('role') == 'system'])
            user_tokens = self._estimate_tokens([msg for msg in messages if msg.get('role') == 'user'])
            assistant_tokens = self._estimate_tokens([msg for msg in messages if msg.get('role') == 'assistant'])
        ...
        for i, obs_display in enumerate(all_observations_for_display):
            if '.aacode/context/' in obs_display:
                # 检查对应的完整观察是否也包含归档路径
                if i < len(all_observations) and '.aacode/context/' in all_observations[i]:
                    # 提取归档文件名
                    import re
                    archive_match = re.search(r'\.aacode/context/([^\s]+\.txt)', obs_display)
                    if archive_match:
                        archive_file = archive_match.group(1)
                        print(f"✅ 归档路径一致性：动作{i+1}归档到 {archive_file}")
                else:
                    print(f"⚠️  归档路径不一致：动作{i+1}的简化版本包含归档路径，但完整版本可能丢失")
